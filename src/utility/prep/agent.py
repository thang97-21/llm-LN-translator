"""
DeepSeekPrepAgent — cached multi-turn context.xml preparation.

The active path preserves the Librarian context shell, full JP volume text,
optional series bible, and metadata-search evidence in one stable system
prefix. It returns one JSON node per generated block in a persisted sequential
conversation, then converts and assembles the nodes into context.xml
atomically. No Gemini, ChromaDB, or main-pipeline subprocess is involved.

The legacy unified and parallel callers remain available only as explicitly
selected recovery paths. Prep-specific conversation history is kept separate
from chapter-translation history so a prior ledger cannot be misread.
"""
...
from src.utility.prep.block_prompts import MULTITURN_BLOCK_ORDER
...
# Sixteen blocks are generated by the prep conversation. The seventeenth prep
# data block, opf_metadata, is already populated by the Librarian and is
# passed through unchanged. Runtime-owned blocks stay pending outside this
# accounting: character_attribute_anchors and translation_inheritance.
PREP_BLOCK_NAMES = tuple(MULTITURN_BLOCK_ORDER)
PREP_DATA_BLOCK_NAMES = ("opf_metadata", *PREP_BLOCK_NAMES)
RUNTIME_BLOCK_NAMES = ("character_attribute_anchors", "translation_inheritance")
# Compatibility alias for the parallel path's active-block logging.
_BLOCK_NAMES = PREP_BLOCK_NAMES
...
def _finalize_and_write(
    ...
    *,
    ...
    opf_populated = root.find("opf_metadata") is not None
    blocks_populated_count = int(opf_populated) + sum(
        1 for name in PREP_BLOCK_NAMES if not _is_pending(root, name)
    )
    manifest.setdefault("pipeline_state", {})["prep"] = {
        "status": "completed",
        "timestamp": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "blocks_populated": blocks_populated_count,
        "blocks_total": len(PREP_DATA_BLOCK_NAMES),
        "model_turns": len(PREP_BLOCK_NAMES),
        "runtime_owned_blocks": list(RUNTIME_BLOCK_NAMES),
    }
    ...
    blocks_populated = (["opf_metadata"] if opf_populated else []) + [
        name for name in PREP_BLOCK_NAMES if not _is_pending(root, name)
    ]
    return {
        ...
        "blocks_pending": [n for n in PREP_BLOCK_NAMES if n not in blocks_populated],
        "blocks_total": len(PREP_DATA_BLOCK_NAMES),
        "model_turns": len(PREP_BLOCK_NAMES),
        "runtime_owned_blocks": list(RUNTIME_BLOCK_NAMES),
    }
...
def _run_cache_loop_prep(volume_id: str, series_id: Optional[str], prep_cfg: Dict[str, Any], provider: str) -> Dict[str, Any]:
    """Run the JSON-fragment cache loop and atomically merge validated XML."""
    from src.Deepseek.prep import prep_cache_client as cache_client
    ...
    api_style = "anthropic" if provider == "minimax" else "openai"
    model = str(provider_cfg.get("model", "MiniMax-M3" if provider == "minimax" else "glm-5.3"))
    args = type("CacheArgs", (), {})()
    args.vol_id = volume_id
    args.series_id = series_id or ""
    args.provider = provider
    args.blocks = None
    args.web_results_file = ""
    args.thinking_dir = str(work_dir / "THINKING" / "prep")
    args.base_url = str(provider_cfg.get("endpoint", "https://api.minimax.io/anthropic" if provider == "minimax" else "https://api.z.ai/api/paas/v4"))
    args.api_key_env = str(provider_cfg.get("api_key_env", "MINIMAX_API_KEY" if provider == "minimax" else "ZAI_API_KEY"))
    args.api_style = api_style
    args.model = model
    args.max_tokens = int(provider_cfg.get("max_output_tokens", cache_client.MODEL_PROFILES.get(model, {}).get("max_output", 131072)))
    args.reasoning_split = False if provider == "glm" else True
    args.no_reasoning_split = provider == "glm"
    args.reasoning_effort = str(provider_cfg.get("reasoning_effort", "high" if provider == "glm" else "max"))
    args.prefix_file = str(cache_client.REPO / ".dsh" / "prep" / ("prefix_glm.md" if provider == "glm" else "prefix.md"))
    args.fallback_base_url = str(prep_cfg.get("endpoint", "https://api.deepseek.com"))
    args.fallback_api_key_env = str(prep_cfg.get("api_key_env", "DEEPSEEK_API_KEY"))
    args.fallback_api_style = "openai"
    args.fallback_model = str(prep_cfg.get("model", "deepseek-v4-pro"))
    args.fallback_max_tokens = int(prep_cfg.get("max_output_tokens", 256000))
    args.cache_mode = str((provider_cfg.get("cache") or {}).get("mode", "passive"))
    args.retries = 1
    args.retry_delay = 2.0
    args.timeout = int(provider_cfg.get("http_timeout_seconds", 900))

    prefix = cache_client.PREFIX_FILE.read_text(encoding="utf-8")
    source_package, _ = cache_client.build_source_package(work_dir)
    fragments, _, failed, _ = cache_client.run_blocks(args, prefix, source_package, work_dir, fallback=False)
    if failed:
        raise PrepError(f"{provider} cache-loop prep failed for blocks: {', '.join(failed)}")

    root = ET.fromstring(existing_context_xml)
    for block in cache_client.BLOCKS:
        if block["name"] == "opf_metadata" and provider == "glm":
            continue  # left as extraction populated it; not re-fetched for GLM
        fragment = fragments.get(block["name"])
        if not fragment or fragment.get("status") != "completed" or not fragment.get("xml"):
            raise PrepError(f"{provider} returned no completed XML fragment for {block['name']}.")
        try:
            element = ET.fromstring(str(fragment["xml"]))
        except ET.ParseError as exc:
            raise PrepError(f"{provider} returned malformed XML for {block['name']}: {exc}") from exc
        if element.tag != block["name"]:
            raise PrepError(f"{provider} returned <{element.tag}> for {block['name']}.")
        _replace_prep_block(root, element)
    ...
    return _finalize_and_write(work_dir, context_path, root, resolved_series_id=resolved_series_id, chapters=chapters, bible_match=bible_match, generated_by=f"{provider}_prep_cache")


def _replace_prep_block(root: ET.Element, new_element: ET.Element) -> None:
    for index, child in enumerate(root):
        if child.tag == new_element.tag:
            root.remove(child)
...
def run_prep(
    ...
    *,
    ...
    force_rerun re-asks every block on the multi-turn path even where a
    completed artifact exists. Only that path resumes per block, so the flag
    is inert for the other two — they have nothing partial to resume from.
    """
    prep_cfg = get_config_section("prep")

    selected_provider = str(prep_cfg.get("provider", "deepseek")).strip().lower()
    if selected_provider == "openai":
        from src.OpenAI.prep import run_openai_prep

        return run_openai_prep(
            volume_id, series_id=series_id, force_rerun=force_rerun
        )
    if selected_provider in {"minimax", "glm"}:
        return _run_cache_loop_prep(volume_id, series_id, prep_cfg, selected_provider)

    multi_turn_cfg = prep_cfg.get("multi_turn", {}) or {}
    parallel_cfg = prep_cfg.get("parallel", {}) or {}
    if multi_turn_cfg.get("enabled", False) and parallel_cfg.get("enabled", False):
        raise PrepError(
            "prep.multi_turn and prep.parallel are mutually exclusive; "
            "disable one strategy before running prep to avoid duplicate API spend."
        )
