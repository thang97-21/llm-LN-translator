...
class EPUBExtractor:
    ...
    def extract(self, epub_path: Path, volume_id: Optional[str] = None, force_rerun: bool = False) -> ExtractionResult:
        """
        ...
        Args:
            ...
            force_rerun: When True, a re-extraction over a workspace that already
                has translations proceeds automatically into a NEW derived volume
                directory (old output preserved) without prompting. This is the
                mode launchers/frontends use — a blocking prompt under a TUI can
                never be answered, and the old auto-default silently failed the run.
        ...
        if not force_rerun:
            force_rerun = os.environ.get("MTL_EXTRACT_FORCE_RERUN", "").strip().lower() in {"1", "true", "yes", "on"}
        ...
        if work_dir.exists():
            ...
            if existing_translations:
                ...
                if force_rerun:
                    # Unattended/frontend mode: proceed into a NEW derived volume
                    # directory automatically (option 2, the data-safe choice).
                    # The old workspace — including its translations — is left
                    # untouched; nothing is overwritten.
                    print("  -> auto-continuing (--force-rerun): creating a NEW volume ID; old extraction preserved.")
                else:
