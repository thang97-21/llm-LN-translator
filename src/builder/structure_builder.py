...
class StructureBuilder:
    ...
    def create_cover_xhtml(
        ...
        """
        # Guardrail: normalize cover_image to a plain string.
        # Prevents dict/list leaks (from spine metadata or corrupted manifests) from
        # producing broken src="../Images/{'file': ...}" in the output XHTML.
        if not isinstance(cover_image, str) or not cover_image.strip():
            import warnings
            warnings.warn(
                f"[CORRUPTION GUARD] create_cover_xhtml received cover_image={repr(cover_image)!r} "
                f"(type={type(cover_image).__name__}); falling back to 'cover.jpg'"
            )
