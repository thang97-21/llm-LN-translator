...
class EPUBPackager:
    ...
    def _add_epub_contents(epub_zip: zipfile.ZipFile, working_dir: Path) -> None:
        ...
        for file_path in working_dir.rglob('*'):
            # Skip mimetype (already added), directories, and A3 sentinel files
            if file_path.name == 'mimetype' or file_path.is_dir():
                continue
            # A3: Skip .FAILED.md sentinel files
            if file_path.suffix == '.md' and file_path.name.endswith('.FAILED.md'):
                continue
