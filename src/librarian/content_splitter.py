...
class KodanshaSplitter:
    ...
    DEFAULT_HEADING_PATTERNS = [
        ...
        r'^###\s+[一二三四五六七八九十百千]+章.*$',               # Bare N章 with text
        r'^###\s+[一二三四五六七八九十百千]+章[（(][前後][）)].*$',  # Split chapters: 三章（前）
        r'^###\s+[序終]章.*$',                                     # 序章/終章
        r'^###\s+プロローグ.*$',                                    # プロローグ with optional suffix
        r'^###\s+エピローグ.*$',                                    # エピローグ with optional suffix
        r'^###\s+あとがき$',                                       # Afterword (exact)
    ]
