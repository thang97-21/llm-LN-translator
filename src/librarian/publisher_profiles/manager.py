...
class ContentConfig:
    ...
    # Filename patterns for chapter-heading ("midashi") images. Some publishers
    # render the chapter title as an image rather than text; a page that leads
    # with such an image marks a chapter boundary during spine-fallback
    # detection. Extends the built-in defaults (see
    # PublisherProfileManager.CHAPTER_HEADING_IMAGE_PATTERNS).
    chapter_title_image_patterns: List[str] = field(default_factory=list)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'ContentConfig':
        ...
        return cls(
            ...
            chapter_title_image_patterns=chapter_detection.get("title_image_patterns", []),
...
class PublisherProfileManager:
    ...
    # Chapter-heading ("midashi" / 見出し) image conventions. Publishers such as
    # KADOKAWA render each chapter's title as an image (m-###.jpg / midashi###.jpg)
    # rather than as text, and ship a minimal cover+colophon navigation. These
    # images are already excluded from story illustrations (see
    # HARD_EXCLUDE_IMAGE_PATTERNS); this dedicated set lets spine-fallback chapter
    # detection treat a page that *leads* with one as a chapter boundary, so
    # image-titled chapters don't collapse into a single spine group.
    CHAPTER_HEADING_IMAGE_PATTERNS: Tuple[re.Pattern, ...] = (
        re.compile(r'^m[-_]?\d{2,4}\.(jpe?g|png)$', re.IGNORECASE),
        re.compile(r'^midashi\d+\.(jpe?g|png)$', re.IGNORECASE),
    )
    ...
    def is_chapter_heading_image(self, filename: str, publisher: str = None) -> bool:
        """Return True if *filename* is a chapter-heading ("midashi") image.

        Publishers such as KADOKAWA render chapter titles as heading images
        (m-###.jpg / midashi###.jpg) instead of as text. Spine-fallback chapter
        detection uses a page that *leads* with one of these images as a chapter
        boundary, which keeps image-titled chapters from collapsing into a single
        spine group. Built-in conventions live in CHAPTER_HEADING_IMAGE_PATTERNS;
        publishers may add more via the profile's chapter_title_image_patterns.
        """
        if not filename:
            return False
        base = filename.split('/')[-1].strip()
        if not base:
            return False
        for pattern in self.CHAPTER_HEADING_IMAGE_PATTERNS:
            if pattern.match(base):
                return True
        config = self.get_content_config(publisher)
        for pattern_str in getattr(config, "chapter_title_image_patterns", None) or []:
            try:
                if re.match(pattern_str, base, re.IGNORECASE):
                    return True
            except re.error:
                continue
        return False
    ...
    def get_chapter_title_patterns(self, publisher: str = None) -> List[re.Pattern]:
        ...
        baseline_patterns = [
            ...
            r'^[一二三四五六七八九十百千]+章',  # Kodansha-style bare N章 (no 第 prefix)
            r'^[序終]章',  # 序章/終章 (opening/closing chapter)
            ...
            r'^Epilogue',
            r'^Interlude',
            r'^\d+\s+\S',  # Numbered chapter with text (Kodansha: "１ text...")
            r'^[１２３４５６７８９０]+\s+\S',  # Full-width numbered chapter with text
        ]
