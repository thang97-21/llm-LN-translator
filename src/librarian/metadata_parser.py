...
class MetadataParser:
    ...
    def parse_opf(self, opf_path: Path) -> BookMetadata:
        ...
        # P-7: Strip Japanese publisher author-prefix convention: [著者名] タイトル
        raw_title = self._get_dc_text(meta_elem, "title")
        metadata.title = re.sub(r"^\[[^\]]+\]\s*", "", raw_title) if raw_title else ""
        ...
        # Auto-detect sequel if series_index not found in OPF metadata
        # This enables sequel mode for volumes without explicit series metadata
        if not metadata.series_index and metadata.title:
            sequel_info = detect_sequel(metadata.title, opf_path)
            if sequel_info:
                metadata.series = sequel_info[0]
                metadata.series_index = sequel_info[1]
                print(f"[INFO] Auto-detected sequel: series='{metadata.series}', index={metadata.series_index}")
