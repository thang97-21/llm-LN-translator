"""GLM-owned context.xml extraction for chapter guidance.

This module intentionally owns its parsers. The GLM route must not change when
the Qwen route evolves, even though both routes currently consume the same
context.xml schema.
"""
...
def parse_name_map(context_xml: Optional[str]) -> Dict[str, str]:
    """Build a bidirectional JP↔EN lookup from context.xml's <name_map>."""
    root = _parse(context_xml)
...
def parse_character_roster_handles(context_xml: Optional[str]) -> Dict[str, str]:
    ...
    for character in block.findall("character"):
        ...
        mapping[canonical] = jp_to_en.get(jp_name, canonical) if jp_name else canonical
    return mapping
...
def resolve_voice_aliases(
    ...
    """Add canonical-name aliases to the handle-keyed voice profiles."""
...
def _parse(context_xml: Optional[str]) -> Optional[ET.Element]:
    ...
    try:
        ...
        logger.warning("[GLM-CONTEXT] context.xml parse failed: %s", exc)
        return None
...
def parse_volume_type(context_xml: Optional[str]) -> str:
    ...
    return vt.text.strip() if vt is not None and vt.text else ""
