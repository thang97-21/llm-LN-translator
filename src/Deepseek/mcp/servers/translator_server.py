...
from src.Deepseek.mcp.runtime import MCPRuntimeError, load_manifest, resolve_volume_dir
...
def register_translator_tools(mcp: object, cfg: MCPConfig) -> None:
    ...
    def translate_chapter(
        ...
        translator = get_translator_class()(
    ...
    def run_translator(
        ...
        # Delegate to translate_volume() so the MCP path benefits from the
        # same completed-chapter filtering as the CLI path — no duplicated
        # chapter-gathering logic.
        results = get_volume_translator()(
            volume_id,
            chapters=chapters,
            thinking_log_enabled=thinking_log,
            dry_run=dry_run,
        )
