...
One persisted conversation with a stable system prefix (prep policy + context
shell + full JP source + series bible + metadata-search evidence). Fifteen
JSON-node turns, one per generated `context.xml` block, in fixed schema
order; deterministic JSON-to-XML assembly after every turn validates. Every
model turn writes a row to the per-volume cost ledger under
`work/<volume_id>/LOG/`.
